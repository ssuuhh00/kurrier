...
class LatticePlanner:
    ...
    def relative_to_absolute2(self, rel_x, rel_y, rel_z):
        """
        장애물의 상대 좌표를 절대 좌표로 변환합니다.
        """
        # 차량의 현재 위치와 헤딩 정보를 가져옵니다.
        ...
        wheel_base = -3.15  # 차량의 wheel base (m)
        front_axle_x = ego_x + cos(heading) * wheel_base
        front_axle_y = ego_y + sin(heading) * wheel_base   
        # 변환 행렬을 사용하여 상대 좌표를 절대 좌표로 변환
        abs_x = front_axle_x + cos(heading) * (rel_x + 0.25) - sin(heading) * rel_y
        abs_y = front_axle_y + sin(heading) * (rel_x + 0.25) + cos(heading) * rel_y
        abs_z = ego_z + rel_z  # z는 일반적으로 사용되지 않지만 포함
